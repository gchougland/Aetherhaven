---
name: "Corin Mosscup"
description: "Quests, friendship, schedule, and gifts for Corin Mosscup."
author: Hexvane
npcRoleId: Aetherhaven_Innkeeper
---

![](wiki/villager_innkeeper.png)

**Corin Mosscup** is usually found at **the inn** when they are on shift.

### Quests

#### A Place To Breathe
Corin wants a small park with benches so townsfolk can sit outside and feel a little lighter between chores.

1. Take the park plot token from Corin.
2. Stake and build the park.
3. Find Corin at the inn and close the quest in dialogue.

#### Somewhere To Celebrate
The park gave the town a place to sit. Corin wants a proper square next, so the whole town has somewhere to gather when a festival comes around.

1. Take the festival square plot token from Corin.
2. Stake and build the festival square.
3. Find Corin at the inn and close the quest in dialogue.

#### Corin's home
Corin feeds everyone else first. He would like a door that closes on his own mess when the night is done.

1. Take the house plot token from Corin.
2. Stake and build the house prefab.
3. Assign Corin as resident on the house management block.
4. Find Corin and close the quest in dialogue.

### Friendship rewards

Visit them after your friendship reaches each level to pick up the reward through dialogue.

- **Friendship 50:** teaches you to craft **Banquet Table**
- **Friendship 75:** visit them for their next friendship scene
- **Friendship 100:** visit them for their next friendship scene

## Weekly routine


Times are in world hours. Most days follow the same rhythm; Sunday can include a short visit to the altar.

### Monday
- **06:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home

### Tuesday
- **06:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home

### Wednesday
- **06:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home

### Thursday
- **06:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home

### Friday
- **06:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home

### Saturday
- **06:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home

### Sunday
- **06:00** — the inn
- **08:00** — the Gaia altar
- **09:00** — the inn
- **11:00** — the inn commons
- **13:00** — the inn
- **16:45** — the town park
- **18:15** — the inn
- **20:00** — home
