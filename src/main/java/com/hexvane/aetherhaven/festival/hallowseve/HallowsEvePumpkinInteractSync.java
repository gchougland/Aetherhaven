...
/** Turns the F prompt on only when the jack o lantern is ready to pop. */
public final class HallowsEvePumpkinInteractSync {
    private HallowsEvePumpkinInteractSync() {}
    ...
    private static void ensureBurstUse(
        @Nonnull Ref<EntityStore> ref,
        @Nonnull CommandBuffer<EntityStore> commandBuffer
    ) {
    ...
    private static boolean isBurstUse(@javax.annotation.Nullable Interactions existing) {
        return existing != null
            && HallowsEveIds.ROOT_INTERACTION_BURST.equals(existing.getInteractionId(InteractionType.Use))
            && HallowsEveIds.INTERACTION_HINT.equals(existing.getInteractionHint());
    }

    @Nonnull
    private static Interactions burstInteractions() {
        Interactions interactions =
            new Interactions(Map.of(InteractionType.Use, HallowsEveIds.ROOT_INTERACTION_BURST));
        interactions.setInteractionHint(HallowsEveIds.INTERACTION_HINT);
        return interactions;
    }
}
