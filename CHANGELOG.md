...
- **Gaia statue appearances** Choose from 11 base-game statue looks. Your choice carries over to saved and marketplace builds.
- **Town backups** Regular world backups now include a recent copy of your town data and custom content.
...
- **Tree Climbing Festival** Mid summer holiday at the festival square. Join a climb race at the attendant, race up the big tree to the pink crystal, earn summer festival tickets for a fast time, check the shared leaderboard, and buy rare saplings plus bush, beehive, jungle sapling, and rafflesia wardrobe hats from the merchant.
...
- **Town look** After you start a town and pick a difficulty, choose a look for building quests. Core and Stormwind are ready right away. Jimmy's Village, Fairy Tale, Coastal Ruins, and Slate Fjord can be downloaded from the marketplace first. Building quests then give plot tokens that match that look when they can, or the core look if not. Use `/aetherhaven town style` to change it later.
...
- **Shop spot status panel** — Moved to the top left so it no longer covers the clock.
- **Town NPC death messages** — When a town linked NPC dies, members see coordinates and a short cause line.
- **Plot creator community submit** — Submit to community starts unchecked by default. Server config can still opt in by default.

### Fixed

- **House assignment picker** — Portal visitors and hired guards only appear when you can assign them to a house, after accepting their house quest.
- **Battle horn sound** — Nearby players hear the horn when someone blows it.
- **Plot signs over water** — Plot signs on boat plots sit above the water surface.
- **Prefab materials for tree trunks** — Buildings using tree trunks now ask for gatherable trunk blocks, not the full block version.
- **Villager energy at work** — Finishing work at a POI no longer refills hunger, energy, or fun. Rest comes from beds and food spots instead.
- **Community submit duplicate buildings** — Submitting from the plot creator no longer creates duplicate entries on the crafting bench.
- **Guard combat** — Hired guards engage hostiles more reliably, fight back when hit, and defend you in follow mode. Multiple following guards join the same fight.
- **Raid quest spawning** — Raid waves search wider for dry land on the surface and try another direction when blocked by water or cliffs.
